...
setupLoan(ApplyContext& ctx, beast::Journal const& j)
{
    ...
    auto const originationDeltas =
        loanOriginationDeltas(vaultSle, principalRequested, state.interestDue);
    auto const newDebtTotal = brokerSle->at(sfDebtTotal) + originationDeltas.debtTotalDelta;
    ...
    beast::Journal const& j)
{
    ...
    vaultTotalProxy += originationDeltas.assetsTotalDelta;
    ...
    adjustImpreciseNumber(
        brokerSle->at(sfDebtTotal), originationDeltas.debtTotalDelta, vaultAsset, vaultScale);
