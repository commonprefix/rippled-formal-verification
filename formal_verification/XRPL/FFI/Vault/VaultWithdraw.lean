...
def lean_shares_to_assets_withdraw (lv : LawfulVault) (shares : STAmount) (waiveUnrealizedLoss : UInt8) : Except Error STAmount :=
  lv.sharesToAssetsWithdraw shares (waiveUnrealizedLoss != 0)
...
def lean_vault_withdraw (lv : LawfulVault) (amount : WithdrawAmount) (waiveUnrealizedLoss : UInt8) : Except Error WithdrawResult :=
  lv.withdraw amount (waiveUnrealizedLoss != 0)
...
def lean_withdraw_result_vault (r : WithdrawResult) : LawfulVault := r.vault'
