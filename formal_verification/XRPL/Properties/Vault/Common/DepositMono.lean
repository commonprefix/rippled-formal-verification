...
lemma deposit_nonempty_chain_facts (lv : LawfulVault) (hne : lv.assetsTotal.mantissa_ ≠ 0)
    ...
    ∃ (amountN navN P T0 T : Number),
      ...
      lv.sharesTotal.mantissa_ ≠ 0 ∧
      lv.sharesTotal.operator_mul amountN .to_nearest = .ok P ∧
  ...
  obtain ⟨_, _, _, hnavpos⟩ := LawfulVault.depositNav_facts lv navN hne hnavm hnavN_eq
...
lemma deposit_empty_chain_facts (lv : LawfulVault) (hscale : lv.scale.toNat ≤ 18)
    (amount shares : STAmount) (hc : amount.Canonical) (hpos : 0 < amount.toRat)
    (hmz : lv.assetsTotal.mantissa_ = 0)
    (hok : assetsToSharesDeposit lv amount = .ok shares) (hnz : shares.isZero = false) :
    ∃ (n1 n2 : Number),
      n1.isNormalized ∧ n1.negative_ = false ∧
      n1.toRat = amount.toRat * (10 : ℚ) ^ (lv.scale.toNat : ℤ) ∧
  ...
  obtain ⟨M, e1, hdn, hMval, hMnorm⟩ :=
    doNormalize_largeRange_exact false amount.mantissa (amount.exponent + (lv.scale.toNat : ℤ))
      .to_nearest hmv1 hmax
      (by show minExponent + 18 ≤ amount.exponent + (lv.scale.toNat : ℤ)
          have hs : (0 : ℤ) ≤ (lv.scale.toNat : ℤ) := by positivity
          unfold minExponent; omega)
      (by show amount.exponent + (lv.scale.toNat : ℤ) ≤ maxExponent - 1
          have hs : (lv.scale.toNat : ℤ) ≤ 18 := by exact_mod_cast hscale
          unfold maxExponent; omega)
  have hn1eq : n1 = ⟨false, M, e1⟩ := by
    have hdn' : Number.normalized false amount.mantissa (amount.exponent + (lv.scale.toNat : ℤ))
  ...
  show (amount.mValue.toNat : ℚ) * 10 ^ (amount.mOffset + (lv.scale.toNat : ℤ))
     = (amount.mValue.toNat : ℚ) * 10 ^ amount.mOffset * 10 ^ (lv.scale.toNat : ℤ)
...
lemma assetsToSharesDeposit_empty_mono (lv : LawfulVault) (hscale : lv.scale.toNat ≤ 18)
  ...
  obtain ⟨n1₁, n2₁, hn1n₁, hn1neg₁, hn1v₁, htr₁, hn2n₁, hden₁, hsh₁⟩ :=
    deposit_empty_chain_facts lv hscale amount₁ shares₁ hc₁ hpos₁ hmz hok₁ hnz₁
  obtain ⟨n1₂, n2₂, hn1n₂, hn1neg₂, hn1v₂, htr₂, hn2n₂, hden₂, hsh₂⟩ :=
    deposit_empty_chain_facts lv hscale amount₂ shares₂ hc₂ hpos₂ hmz hok₂ hnz₂
...
`⌊amount · 10^scale⌋` map). -/
lemma assetsToSharesDeposit_mono (lv : LawfulVault)
    (amount₁ amount₂ shares₁ shares₂ : STAmount)
    (hc₁ : amount₁.Canonical) (hpos₁ : 0 < amount₁.toRat)
    (hc₂ : amount₂.Canonical) (hpos₂ : 0 < amount₂.toRat)
    (hok₁ : assetsToSharesDeposit lv amount₁ = .ok shares₁) (hnz₁ : shares₁.isZero = false)
  ...
  · exact assetsToSharesDeposit_empty_mono lv lv.wf.scale_le amount₁ amount₂ shares₁ shares₂
      hc₁ hpos₁ hc₂ hpos₂ hmz hok₁ hnz₁ hok₂ hnz₂ hle
  have hne : lv.assetsTotal.mantissa_ ≠ 0 := hmz
  obtain ⟨aN₁, navN₁, P₁, T0₁, T₁, hav₁, haN₁, ham₁, hnav₁, hnavn₁, hnavm₁, hnavp₁, hSTm₁,
      hP₁, hPn₁, hPm₁, hT0₁, hT0n₁, hT0m₁, hT0p₁, hT₁, hTn₁, hTm₁, hTden₁, hsh₁⟩ :=
    deposit_nonempty_chain_facts lv hne amount₁ shares₁ hc₁ hpos₁ hok₁ hnz₁
  obtain ⟨aN₂, navN₂, P₂, T0₂, T₂, hav₂, haN₂, ham₂, hnav₂, hnavn₂, hnavm₂, hnavp₂, hSTm₂,
      hP₂, hPn₂, hPm₂, hT0₂, hT0n₂, hT0m₂, hT0p₂, hT₂, hTn₂, hTm₂, hTden₂, hsh₂⟩ :=
    deposit_nonempty_chain_facts lv hne amount₂ shares₂ hc₂ hpos₂ hok₂ hnz₂
  ...
  have hSTneg := Number.negative_false_of_pos lv.sharesTotal hSTpos
  ...
  have hT0le : T0₁.toRat ≤ T0₂.toRat :=
    Number.mul_div_num_mono lv.sharesTotal navN₁ aN₁ aN₂ P₁ P₂ T0₁ T0₂
      lv.wf.sharesTotal_norm hSTm₁ hSTneg hnavn₁ hnavm₁ hnavneg
...
private lemma roundedDepositAmount_roundTo (lv : LawfulVault) (amountDeposit roundedAmount : STAmount)
    (h : lv.roundedDepositAmount amountDeposit = .ok (.rounded roundedAmount)) :
    roundToVaultExponent amountDeposit lv.assetsTotal = .ok roundedAmount := by
  unfold LawfulVault.roundedDepositAmount at h
  simp only [] at h
...
theorem LawfulVault.deposit_shares_monotone_proof (lv : LawfulVault)
  ...
  have haeq₁ : am₁ = roundedAmount₁ :=
    Except.ok.inj (hround₁.symm.trans (roundedDepositAmount_roundTo lv _ _ hrounded₁))
  have haeq₂ : am₂ = roundedAmount₂ :=
    Except.ok.inj (hround₂.symm.trans (roundedDepositAmount_roundTo lv _ _ hrounded₂))
  ...
  have hsi₂ : r₂.sharesIssued = sh₂ := by rw [hr₂]; exact hseq₂
  rw [hsi₁, hsi₂]
  exact assetsToSharesDeposit_mono lv am₁ am₂ sh₁ sh₂
