...
lemma deposit_nonempty_chain_facts (hv : v.Lawful) (hne : v.assetsTotal.mantissa_ ≠ 0)
...
lemma deposit_empty_chain_facts (hscale : v.scale.toNat ≤ 18)
    (amount shares : STAmount) (hc : amount.Canonical) (hpos : 0 < amount.toRat)
...
lemma assetsToSharesDeposit_empty_mono (hscale : v.scale.toNat ≤ 18)
...
`⌊amount · 10^scale⌋` map). -/
lemma assetsToSharesDeposit_mono (hv : v.Lawful)
    (amount₁ amount₂ shares₁ shares₂ : STAmount)
    (hc₁ : amount₁.Canonical) (hpos₁ : 0 < amount₁.toRat)
    (hc₂ : amount₂.Canonical) (hpos₂ : 0 < amount₂.toRat)
    (hok₁ : assetsToSharesDeposit v amount₁ = .ok shares₁) (hnz₁ : shares₁.isZero = false)
  ...
  · exact assetsToSharesDeposit_empty_mono v hv.wf.scale_le amount₁ amount₂ shares₁ shares₂
  ...
  have hT0le : T0₁.toRat ≤ T0₂.toRat :=
    Number.mul_div_num_mono v.sharesTotal navN₁ aN₁ aN₂ P₁ P₂ T0₁ T0₂
      hv.wf.sharesTotal_norm hSTm₁ hSTneg hnavn₁ hnavm₁ hnavneg
...
private lemma roundedDepositAmount_roundTo (amountDeposit roundedAmount : STAmount)
...
theorem Vault.deposit_shares_monotone_proof (hv : v.Lawful)
  ...
  obtain ⟨am₁, aD₁, sC₁, _, _, _, _, _, hround₁, _, _, _, _, hcomp₁, _, _, _, _, _, _, hr₁⟩ :=
  ...
  have hsi₂ : r₂.sharesIssued = sh₂ := by rw [hr₂]; exact hseq₂
  rw [hsi₁, hsi₂]
  exact assetsToSharesDeposit_mono v hv am₁ am₂ sh₁ sh₂
