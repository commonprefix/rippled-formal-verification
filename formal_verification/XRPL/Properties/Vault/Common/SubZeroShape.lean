...
lemma operator_div_numerator_ne_zero_sz (x y result : Number) (mode : rounding_mode)
  ...
  · rw [if_pos hxe] at hok
    have hxr : x = result :=
      Except.ok.inj (show (Except.ok x : Except String Number) = .ok result from hok)
    rw [← hxr]; exact hxz
  · rw [if_neg hxe] at hok
    ...
    have : Number.zero = result :=
      Except.ok.inj (show (Except.ok Number.zero : Except String Number) = .ok result from hok)
    rw [← this]; rfl
