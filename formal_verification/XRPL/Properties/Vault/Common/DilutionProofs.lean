...
`Vault.ReachableFromIn` lives in `ReachableDefs`). The induction carries the
...
theorem Vault.deposit_withdrawNav_change (v : Vault) (amountDeposit : STAmount) (isDonation : Bool)
...
donation. -/
theorem Vault.deposit_donation_sharesTotal_toExact (v : Vault) (amountDeposit : STAmount)
    (r : DepositResult) (hok : v.deposit amountDeposit true = .ok r) (herr : r.error = none) :
...
theorem Vault.deposit_donation_no_dilution_proof (v : Vault) (amountDeposit : STAmount) (r : DepositResult)
  ...
  · -- integral donation: passthrough + exact in-domain add
    ...
    obtain ⟨hAT', -⟩ :=
      Vault.deposit_vault_updates_integral_proof v amountDeposit true r hnt hIC hty hdenA hdenAv
        hok herr hbound'
  ...
  · -- FRACTIONAL BRANCH (postScale grid). `donation_grid_bound` delivers, for the single
    ...
    have hAnn : 0 ≤ v.toExact.assetsTotal := v.exact.assetsTotal_nonneg
    ...
    have hru := (Vault.deposit_vault_updates v amountDeposit true hcanon hpos r hok herr).1
    rw [hAmt'] at hru
...
theorem Vault.deposit_nonneg_and_update_lower (v : Vault) (amountDeposit : STAmount) (r : DepositResult)
    (hcanon : amountDeposit.Canonical) (hpos : 0 < amountDeposit.toRat)
  ...
  have hcN_nn : 0 ≤ cN.toRat := by
    rw [hval]; exact sharesToAssetsDeposit_nonneg v shares aD hshc hshnt hshpos hsad
  have hAnn : 0 ≤ v.assetsTotal.toRat := v.exact.assetsTotal_nonneg
  have hru := operator_add_nonneg_rounds v.assetsTotal cN at' v.wf.assetsTotal_norm hnorm
    hAnn hcN_nn hat
...
floor. -/
theorem Vault.deposit_charge_lower (v : Vault) (amountDeposit : STAmount) (r : DepositResult)
    (hcanon : amountDeposit.Canonical) (hpos : 0 < amountDeposit.toRat)
  ...
  have hApos : 0 < v.toExact.assetsTotal := by
    ...
    · exact h
    · have hins0 : v.isInsolvent = false := hinsolv rfl
      have hins : v.isInsolvent = true :=
        (Vault.isInsolvent_iff ⟨v.toRawVault, v.wf, (RawVault.valid_iff_exact v.toRawVault v.wf).mpr v.exact⟩).mpr ⟨h.symm, hSnat⟩
  ...
  obtain ⟨Q, hcQ, hidpos, hQnz, hQz⟩ :=
    sharesToAssetsDeposit_charge_nonempty_raw v shares aD hshc hshnt hshpos hmz hsad
...
/-- Proof body of `Vault.deposit_no_dilution`. Under `lossUnrealized = 0`
...
theorem Vault.deposit_no_dilution_proof (v : Vault) (amountDeposit : STAmount) (r : DepositResult)
  ...
  have hupd := Vault.deposit_vault_updates v amountDeposit false hcanon hpos r hok herr
  ...
  -- basic sign facts
  have hAnn : 0 ≤ v.toExact.assetsTotal := v.exact.assetsTotal_nonneg
  have hbounds := Vault.deposit_nonneg_and_update_lower v amountDeposit r hcanon hpos hok herr
  ...
  · -- S = 0 ⇒ assetsTotal = 0 (empty_shares) ⇒ both sides are 0.
    ...
    have hA0 : v.toExact.assetsTotal = 0 := (v.exact.empty_shares hSnat).1
    rw [← hS0, hA0]; simp
  · -- S > 0: apply the composition core with the two raw-stage bounds.
    ...
    · -- charge lower bound at the raw pipeline stage `19/(2^63-3)`.
      exact Vault.deposit_charge_lower v amountDeposit r hcanon hpos hcnz hSpos hok herr
...
the difference. -/
theorem Vault.withdraw_withdrawNav_change (v : Vault) (amount : WithdrawAmount)
...
/-- Proof body of `Vault.withdraw_no_dilution`. Under `lossUnrealized = 0`
...
theorem Vault.withdraw_no_dilution_proof (v : Vault) (amount : WithdrawAmount) (r : WithdrawResult)
  ...
  have hAnn : 0 ≤ A := v.exact.assetsTotal_nonneg
  ...
  · -- non-final exit: the composition core
    ...
    obtain ⟨hp_nn, hideal_nn, hp_up, -⟩ :=
      Vault.sharesToAssetsWithdraw_spec_raw v r.sharesBurned r.assets' false hnn hc hnav hprice
    ...
    obtain ⟨haN_val, haN_norm⟩ :=
      Vault.sharesToAssetsWithdraw_toNumber_facts v r.sharesBurned r.assets' false aN hc hprice hnum_r
    ...
    have hle_AT : p ≤ v.assetsTotal.toRat := le_trans hle_AA v.exact.assetsAvailable_le
    ...
    have hround : RoundsWithin at' (v.assetsTotal.toRat - r.assets'.toRat) .to_nearest
        (6 / (2 ^ 63 - 3 : ℚ)) :=
      Vault.sharesToAssetsWithdraw_sub_stored_raw v r.sharesBurned r.assets' false aN
        v.assetsTotal at' hc hprice hnum_r hp_nn v.wf.assetsTotal_norm
    ...
    have hSpos : 0 < S := by
      ...
      have hApos : 0 < A := lt_of_lt_of_le hAAp v.exact.assetsAvailable_le
      ...
      · exfalso
        have hSnat : v.toExact.sharesTotal = 0 := by
          ...
          exact_mod_cast this
        exact absurd (v.exact.empty_shares hSnat).1 (ne_of_gt hApos)
      · exact hSp
    ...
    have hSharesEq : (r.vault'.toExact.sharesTotal : ℚ) = S - x := by
      have hval := (Vault.withdraw_vault_updates_proof v amount false sta r hp_nn hnn hc hSnt
...
untouched. -/
theorem Vault.clawback_withdrawNav_change (v : Vault) (assets holderShares : STAmount)
    (r : ClawbackResult)
...
side conditions. -/
theorem Vault.clawback_no_dilution_proof (v : Vault) (assets holderShares : STAmount) (r : ClawbackResult)
    (hL : v.toExact.lossUnrealized = 0)
    (hnav : v.WithdrawNavExact false)
    (hc : assets.Canonical)
  ...
  have hSharesEq : (r.vault'.toExact.sharesTotal : ℚ) = S - x :=
    (Vault.clawback_vault_updates_proof' v assets holderShares r hnav hc hSic hSc hSnn
      hok herr).2.2 ⟨hle_x, hSfit⟩
  rw [ge_iff_le, hNav_v, hNav_r, hSharesEq]
  exact hcore

private lemma navEq (lw : Vault)
...
theorem Vault.ReachableFromIn.no_dilution_proof (v : Vault) (n : ℕ) (w : Vault)
    ...
    (hwAV : v.assetsAvailable = v.assetsTotal)
    (h : Vault.ReachableFromIn v w n) :
    w.withdrawNav * (v.toExact.sharesTotal : ℚ) ≥
      v.withdrawNav * (w.toExact.sharesTotal : ℚ) * (1 - depositε) ^ n := by
  -- the ops return `Vault`, so each reachable `u` is lawful by construction
  suffices H : ∀ (u : Vault) (m : ℕ), Vault.ReachableFromIn v u m →
      u.toExact.lossUnrealized = 0 ∧
      u.assetsAvailable = u.assetsTotal ∧
      (0 < (u.toExact.sharesTotal : ℚ) ∨ v.withdrawNav = 0) ∧
      u.withdrawNav * (v.toExact.sharesTotal : ℚ) ≥
        v.withdrawNav * (u.toExact.sharesTotal : ℚ) * (1 - depositε) ^ m by
    exact (H w n h).2.2.2
  intro u m hum
  induction hum with
  | refl =>
    refine ⟨hwL, hwAV, ?_, ?_⟩
    · by_cases hSw : (0 : ℚ) < (v.toExact.sharesTotal : ℚ)
      · exact Or.inl hSw
      · refine Or.inr ?_
        have hSw0 : v.toExact.sharesTotal = 0 := by
          have hle : (v.toExact.sharesTotal : ℚ) ≤ 0 := not_lt.mp hSw
          have hge : (0 : ℚ) ≤ (v.toExact.sharesTotal : ℚ) := by exact_mod_cast Nat.zero_le _
          have : (v.toExact.sharesTotal : ℚ) = 0 := le_antisymm hle hge
          exact_mod_cast this
        have hA0 : v.toExact.assetsTotal = 0 := (v.exact.empty_shares hSw0).1
  ...
  | deposit u' k amount isDonation r prev hrun hcanon hpos hcnz hSsz ih =>
    ...
    obtain ⟨hDnn, hSnn⟩ :=
      Vault.deposit_result_nonneg u' amount isDonation  hcanon (le_of_lt hpos) r hrun
    ...
    have hSw_nn : (0 : ℚ) ≤ (v.toExact.sharesTotal : ℚ) := by exact_mod_cast Nat.zero_le _
    ...
    have hSle : (u'.toExact.sharesTotal : ℚ) ≤ (r.vault'.toExact.sharesTotal : ℚ) := by
      ...
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        have heq := (Vault.deposit_vault_updates u' amount isDonation  hcanon hpos r hrun herr').2.2 hSsz
    ...
    have hstep : r.vault'.withdrawNav * (u'.toExact.sharesTotal : ℚ) ≥
      ...
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        ...
        · subst hdon
          ...
          have hupd := (Vault.deposit_vault_updates u' amount true  hcanon hpos r hrun herr').1
        ...
        · have hnd : isDonation = false := by
          ...
          subst hnd
          exact Vault.deposit_no_dilution_proof u' amount r  hcanon hpos hLu0 hcnz hSsz hrun herr'
      ...
      · rw [pow_succ]
        ...
        have h1 : u'.withdrawNav * (r.vault'.toExact.sharesTotal : ℚ) * (1 - depositε)
            * (v.toExact.sharesTotal : ℚ)
            ≤ r.vault'.withdrawNav * (u'.toExact.sharesTotal : ℚ) * (v.toExact.sharesTotal : ℚ) :=
          mul_le_mul_of_nonneg_right hstep hSw_nn
        have h2 : v.withdrawNav * (u'.toExact.sharesTotal : ℚ) * (1 - depositε) ^ k
            * ((r.vault'.toExact.sharesTotal : ℚ) * (1 - depositε))
            ≤ u'.withdrawNav * (v.toExact.sharesTotal : ℚ)
              * ((r.vault'.toExact.sharesTotal : ℚ) * (1 - depositε)) :=
          mul_le_mul_of_nonneg_right hratio_u hey_nn
        have key : (v.withdrawNav * (r.vault'.toExact.sharesTotal : ℚ)
  ...
  | withdraw u' k amount r prev hrun hnav hcnz hSc hcanon hSnt hSneg hmargin hSfit ih =>
    ...
    have hDnn : 0 ≤ r.assets'.toRat :=
      Vault.withdraw_assets_nonneg u' amount false  hLu0 r hrun hSc hSnt hSneg
    ...
    have hstep : r.vault'.withdrawNav * (u'.toExact.sharesTotal : ℚ) ≥
      ...
      · obtain ⟨hveq, -, -⟩ := Vault.withdraw_error_unchanged u' amount false r hrun herr
        rw [hveq]; nlinarith [mul_nonneg hNu_nn hSu_nn, he_le1]
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        exact Vault.withdraw_no_dilution_proof u' amount r  hLu0 hnav hcnz hnn hcanon hSnt hmargin hSfit hrun herr'
    ...
    have hSrpos : 0 < (u'.toExact.sharesTotal : ℚ) → 0 < (r.vault'.toExact.sharesTotal : ℚ) := by
      ...
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        ...
        · exfalso
          ...
          linarith [hSu_pos]
        · have hupd := (Vault.withdraw_vault_updates_proof u' amount false  sta r hDnn hnn hcanon hSnt
  ...
  | clawback u' k assets holderShares r prev hrun hnav hcanon hSic hSc hSnn hmargin hSfit ih =>
    ...
    have hstep : r.vault'.withdrawNav * (u'.toExact.sharesTotal : ℚ) ≥
      ...
      · obtain ⟨hveq, -, -⟩ := Vault.clawback_error_unchanged u' assets holderShares r hrun herr
        rw [hveq]; nlinarith [mul_nonneg hNu_nn hSu_nn, he_le1]
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        exact Vault.clawback_no_dilution_proof u' assets holderShares r  hLu0 hnav hcanon
    ...
    have hSrpos : 0 < (u'.toExact.sharesTotal : ℚ) → 0 < (r.vault'.toExact.sharesTotal : ℚ) := by
      ...
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        ...
        have hSrq := (Vault.clawback_vault_updates_proof' u' assets holderShares r  hnav hcanon
  ...
  | burnShares u' k sharesDestroyed sharesTotalAmount u'' prev hcan hcanon hSneg hle hnn hden hSfit hburn ih =>
    ...
    have hguard : (u'.sharesTotal.mantissa_ == 0 ||
      ...
      simp only [Vault.canBurnShares] at hcan
