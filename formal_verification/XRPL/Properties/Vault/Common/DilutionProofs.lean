...
`LawfulVault.ReachableFromIn` lives in `ReachableDefs`). The induction carries the
`loss = 0` invariant across steps, so it imports `Unchanged` for the
`*_error_unchanged` theorems and `Preservation` for `*_preserves_unrealized`,
`*_asset_parity`, and the result-nonneg / shape lemmas. -/
...
`lossUnrealized` untouched, so it cancels in the difference. -/
theorem LawfulVault.deposit_withdrawNav_change (lv : LawfulVault) (amountDeposit : STAmount) (isDonation : Bool)
    (r : DepositResult) (hok : lv.deposit amountDeposit isDonation = .ok r)
    (herr : r.error = none) :
    r.vault'.withdrawNav - lv.withdrawNav =
      r.vault'.toExact.assetsTotal - lv.toExact.assetsTotal := by
  ...
  rw [hrv]
  show ((at' : Number).toRat - lv.lossUnrealized.toRat)
      - (lv.assetsTotal.toRat - lv.lossUnrealized.toRat)
      = (at' : Number).toRat - lv.assetsTotal.toRat
  ring
...
theorem LawfulVault.deposit_donation_sharesTotal_eq (lv : LawfulVault) (amountDeposit : STAmount)
    (r : DepositResult) (hok : lv.deposit amountDeposit true = .ok r) (herr : r.error = none) :
    r.vault'.sharesTotal = lv.sharesTotal := by
  obtain ⟨amount, aD, sC, cN, sN, at', av', st', hround, hamz, hshdon, hinsolv, hdon, hcomp,
    hcN, hsN, hat, hav, hst, hmax, hamt, hshr, hrv⟩ :=
    LawfulVault.deposit_success_reduces lv amountDeposit true r hok herr
  obtain ⟨_, hsC⟩ := hdon rfl
  rw [hrv]
  show st' = lv.sharesTotal
...
donation. -/
theorem LawfulVault.deposit_donation_sharesTotal_toExact (lv : LawfulVault) (amountDeposit : STAmount)
    (r : DepositResult) (hok : lv.deposit amountDeposit true = .ok r) (herr : r.error = none) :
    r.vault'.toExact.sharesTotal = lv.toExact.sharesTotal := by
  have h := LawfulVault.deposit_donation_sharesTotal_eq lv amountDeposit r hok herr
  show r.vault'.sharesTotal.toRat.num.toNat = lv.sharesTotal.toRat.num.toNat
  rw [h]
...
/-- Proof body of `LawfulVault.deposit_donation_no_dilution`. The share half is
...
theorem LawfulVault.deposit_donation_no_dilution_proof (lv : LawfulVault) (amountDeposit : STAmount) (r : DepositResult)
    ...
    r.vault'.toExact.sharesTotal = lv.toExact.sharesTotal := by
  refine ⟨?_, LawfulVault.deposit_donation_sharesTotal_toExact lv amountDeposit r hok herr⟩
  have hchange := LawfulVault.deposit_withdrawNav_change lv amountDeposit true r hok herr
  -- Reduce to: the stored assetsTotal strictly increases.
  suffices hAlt : lv.toExact.assetsTotal < r.vault'.toExact.assetsTotal by linarith [hchange]
  ...
  · -- integral donation: passthrough + exact in-domain add
    ...
    have hrd : lv.roundedDepositAmount amountDeposit = .ok (.rounded amountDeposit) :=
      LawfulVault.roundedDepositAmount_integral lv amountDeposit hint hnz
    have hAmt' : r.amountDeposit' = amountDeposit :=
      (LawfulVault.deposit_donation lv amountDeposit amountDeposit r hrd hok herr).1
    have hIC : amountDeposit.IntegralCanonical := (hcanon.1 hint).1
    have hbound' : lv.toExact.assetsTotal + r.amountDeposit'.toRat ≤ 2 ^ 63 - 1 := by
      rw [hAmt']; exact hbound
    obtain ⟨hAT', -⟩ :=
      LawfulVault.deposit_vault_updates_integral_proof lv amountDeposit true r hnt hIC hty hdenA hdenAv
        hok herr hbound'
    show lv.toExact.assetsTotal < r.vault'.toExact.assetsTotal
    show lv.toExact.assetsTotal < r.vault'.assetsTotal.toRat
  ...
  · -- FRACTIONAL BRANCH (postScale grid). `donation_grid_bound` delivers, for the single
    ...
    have hAmt' : r.amountDeposit' = amount := by rw [hamt]; exact hAssetEq
    ...
    obtain ⟨s, hF1, hF2⟩ :=
      LawfulVault.donation_grid_bound lv amountDeposit amount hcanon hfr hpos hround hamz
    have hF2' : lv.toExact.assetsTotal < 2 * 10 ^ 16 * (10 : ℚ) ^ s := hF2
    have hAnn : 0 ≤ lv.toExact.assetsTotal := lv.exact.assetsTotal_nonneg
    have hpow_pos : (0 : ℚ) < (10 : ℚ) ^ s := zpow_pos (by norm_num) _
    have hkey : lv.toExact.assetsTotal * depositε < amount.toRat * (1 - depositε) := by
      rw [depositε_eq]; nlinarith [hF1, hF2', hpow_pos]
    have hru := (LawfulVault.deposit_vault_updates lv amountDeposit true hcanon hpos r hok herr).1
    rw [hAmt'] at hru
    simp only [RoundsWithin, RatValued.toRat] at hru
    have hsum_nn : 0 ≤ lv.toExact.assetsTotal + amount.toRat := add_nonneg hAnn (le_of_lt hamt_pos)
...
theorem LawfulVault.deposit_nonneg_and_update_lower (lv : LawfulVault) (amountDeposit : STAmount) (r : DepositResult)
    (hcanon : amountDeposit.Canonical) (hpos : 0 < amountDeposit.toRat)
    (hok : lv.deposit amountDeposit false = .ok r) (herr : r.error = none) :
    0 < r.sharesIssued.toRat ∧
    (lv.toExact.assetsTotal + r.amountDeposit'.toRat) * (1 - 6 / (2 ^ 63 - 3)) ≤
  ...
  have hamCanon : am.Canonical := by
    rcases roundToVaultExponent_canonical_or_isZero amountDeposit am lv.assetsTotal hcanon hround
      with hc | hz
    · exact hc
    · rw [hz] at hamz; exact absurd hamz (by decide)
  have ham_nn : 0 ≤ am.toRat :=
    RawVault.roundToVaultExponent_nonneg amountDeposit am lv.assetsTotal hcanon (le_of_lt hpos) hround
  ...
  obtain ⟨shares, hats, hshz, hsad, _, hseq⟩ :=
    computeDeposit_success_reduces lv am aD sC (hcomp rfl)
  ...
  have hcN_nn : 0 ≤ cN.toRat := by
    rw [hval]; exact sharesToAssetsDeposit_nonneg lv shares aD hshc hshnt hshpos hsad
  have hAnn : 0 ≤ lv.assetsTotal.toRat := lv.exact.assetsTotal_nonneg
  have hru := operator_add_nonneg_rounds lv.assetsTotal cN at' lv.wf.assetsTotal_norm hnorm
    hAnn hcN_nn hat
  have hT_nn : 0 ≤ lv.assetsTotal.toRat + cN.toRat := add_nonneg hAnn hcN_nn
  simp only [RoundsWithin, RatValued.toRat] at hru
  rw [abs_of_nonneg hT_nn] at hru
  have habs := abs_le.mp hru
  rw [hamt, hshr, hrv]
  refine ⟨by show 0 < sC.toRat; rw [hseq]; exact hshpos, ?_⟩
  show (lv.assetsTotal.toRat + aD.toRat) * (1 - 6 / (2 ^ 63 - 3)) ≤ at'.toRat
  rw [← hval]
  nlinarith [habs.1, hru]

/-- Raw-stage charge lower bound (cross-multiplied) for a successful non-donation
deposit into a nonempty vault (`depositNav = assetsTotal`) whose taken amount is nonzero (`hcnz`, the `isZero = false`
...
floor. -/
theorem LawfulVault.deposit_charge_lower (lv : LawfulVault) (amountDeposit : STAmount) (r : DepositResult)
    (hcanon : amountDeposit.Canonical) (hpos : 0 < amountDeposit.toRat)
    (hcnz : r.amountDeposit'.isZero = false)
    (hSpos : 0 < (lv.toExact.sharesTotal : ℚ))
    (hok : lv.deposit amountDeposit false = .ok r) (herr : r.error = none) :
    lv.toExact.assetsTotal * r.sharesIssued.toRat * (1 - 19 / (2 ^ 63 - 3)) ≤
      r.amountDeposit'.toRat * (lv.toExact.sharesTotal : ℚ) := by
  obtain ⟨am, aD, sC, cN, sN, at', av', st', hround, hamz, hshdon, hinsolv, hdon, hcomp,
    hcN, hsN, hat, hav, hst, hmax, hamt, hshr, hrv⟩ :=
    LawfulVault.deposit_success_reduces lv amountDeposit false r hok herr
  have haDeq : r.amountDeposit' = aD := hamt
  have hSnat : 0 < lv.toExact.sharesTotal := by exact_mod_cast hSpos
  have hApos : 0 < lv.toExact.assetsTotal := by
    rcases lt_or_eq_of_le lv.exact.assetsTotal_nonneg with h | h
    · exact h
    · have hins0 : lv.isInsolvent = false := hinsolv rfl
      have hins : lv.isInsolvent = true :=
        (LawfulVault.isInsolvent_iff ⟨lv.toRawVault, lv.wf, (RawVault.valid_iff_exact lv.toRawVault lv.wf).mpr lv.exact⟩).mpr ⟨h.symm, hSnat⟩
      rw [hins] at hins0; exact absurd hins0 (by decide)
  have hmz : lv.assetsTotal.mantissa_ ≠ 0 :=
  ...
  have hshpos : 0 < shares.toRat :=
    assetsToSharesDeposit_pos lv am shares hamCanon ham_pos hats hshz
  have hideal_eq : lv.idealChargeDeposit shares.toRat =
      lv.toExact.assetsTotal * shares.toRat / (lv.toExact.sharesTotal : ℚ) := by
    unfold RawVault.idealChargeDeposit RawVault.depositNav
    rw [if_neg (ne_of_gt hApos)]
  obtain ⟨Q, hcQ, hidpos, hQnz, hQz⟩ :=
    sharesToAssetsDeposit_charge_nonempty_raw lv shares aD hshc hshnt hshpos hmz hsad
  ...
  have hQm : Q.mantissa_ ≠ 0 := by
    ...
    · exact STAmount.ofNumber_integral_source_ne_zero lv.numericType Q .upward aD hint hcQ haDnz
    · have hfrac : lv.numericType = .fractional := by
        cases hnt : lv.numericType with
        | fractional => rfl
        | integral mv mo ms msh => rw [hnt] at hint; simp [NumericType.isIntegral] at hint
      exact STAmount.ofNumber_iou_mantissa_ne_zero lv.numericType Q .upward aD hfrac hcQ haDnz
  obtain ⟨hQnorm, hQneg, hband⟩ := hQnz hQm
  rw [hideal_eq] at hband
  have hfloor : Q.toRat ≤ aD.toRat :=
    STAmount.ofNumber_upward_ge lv.numericType Q aD hQnorm hQneg hcQ haDnz
  rw [hshr, hamt]
  rw [hseq]
  -- goal: A·shares·(1-δc) ≤ aD·S, from aD ≥ Q ≥ ideal(1-δc) and ideal·S = A·shares
  have hbandle := abs_le.mp hband
  have hAmul : lv.toExact.assetsTotal * shares.toRat / (lv.toExact.sharesTotal : ℚ) *
      (lv.toExact.sharesTotal : ℚ) = lv.toExact.assetsTotal * shares.toRat := by
    field_simp
  have hQlowS :
      lv.toExact.assetsTotal * shares.toRat / (lv.toExact.sharesTotal : ℚ) *
          (1 - 19 / (2 ^ 63 - 3)) * (lv.toExact.sharesTotal : ℚ)
        ≤ Q.toRat * (lv.toExact.sharesTotal : ℚ) :=
    mul_le_mul_of_nonneg_right (by nlinarith [hbandle.1]) (le_of_lt hSpos)
  have hQS : Q.toRat * (lv.toExact.sharesTotal : ℚ) ≤ aD.toRat * (lv.toExact.sharesTotal : ℚ) :=
    mul_le_mul_of_nonneg_right hfloor (le_of_lt hSpos)
  nlinarith [hQlowS, hQS, hAmul]

/-- **Composition core** (with `lossUnrealized = 0`, so
...
/-- **Withdraw/clawback composition core** (with `lossUnrealized = 0`, so
`withdrawNav = assetsTotal = A` for both states). Given the
...
/-- Proof body of `LawfulVault.deposit_no_dilution`. Under `lossUnrealized = 0`
(so `withdrawNav = assetsTotal`), `deposit_withdrawNav_change`
...
theorem LawfulVault.deposit_no_dilution_proof (lv : LawfulVault) (amountDeposit : STAmount) (r : DepositResult)
    (hcanon : amountDeposit.Canonical) (hpos : 0 < amountDeposit.toRat)
    ...
    r.vault'.withdrawNav * (lv.toExact.sharesTotal : ℚ) ≥
      lv.withdrawNav * (r.vault'.toExact.sharesTotal : ℚ) * (1 - depositε) := by
  -- withdrawNav collapses to assetsTotal on both states (loss = 0, preserved).
  have hNav_v : lv.withdrawNav = lv.toExact.assetsTotal := by
    unfold RawVault.withdrawNav; rw [hL]; ring
  have hchange := LawfulVault.deposit_withdrawNav_change lv amountDeposit false r hok herr
  ...
  have hupd := LawfulVault.deposit_vault_updates lv amountDeposit false hcanon hpos r hok herr
  have hSharesEq : (r.vault'.toExact.sharesTotal : ℚ) =
      (lv.toExact.sharesTotal : ℚ) + r.sharesIssued.toRat := hupd.2.2 hSsz
  rw [ge_iff_le, hNav_r, hNav_v, hSharesEq]
  -- basic sign facts
  have hAnn : 0 ≤ lv.toExact.assetsTotal := lv.exact.assetsTotal_nonneg
  have hbounds := LawfulVault.deposit_nonneg_and_update_lower lv amountDeposit r hcanon hpos hok herr
  ...
  rcases eq_or_lt_of_le (by exact_mod_cast Nat.zero_le lv.toExact.sharesTotal :
      (0 : ℚ) ≤ (lv.toExact.sharesTotal : ℚ)) with hS0 | hSpos
  · -- S = 0 ⇒ assetsTotal = 0 (empty_shares) ⇒ both sides are 0.
    have hSnat : lv.toExact.sharesTotal = 0 := by exact_mod_cast hS0.symm
    have hA0 : lv.toExact.assetsTotal = 0 := (lv.exact.empty_shares hSnat).1
    rw [← hS0, hA0]; simp
  · -- S > 0: apply the composition core with the two raw-stage bounds.
    refine deposit_no_dilution_arith lv.toExact.assetsTotal (lv.toExact.sharesTotal : ℚ)
    ...
    · -- charge lower bound at the raw pipeline stage `19/(2^63-3)`.
      exact LawfulVault.deposit_charge_lower lv amountDeposit r hcanon hpos hcnz hSpos hok herr
...
lemma RawVault.stored_sub_payout_raw (stored pn result : Number)
...
pipeline). -/
lemma LawfulVault.sharesToAssetsWithdraw_sub_stored_raw (lv : LawfulVault)
    (shares payout : STAmount) (waive : Bool) (pn stored result : Number)
    (hc : shares.Canonical)
    (hprice : lv.sharesToAssetsWithdraw shares waive = .ok payout)
  ...
  obtain ⟨hpn_val, hpn_norm⟩ :=
    LawfulVault.sharesToAssetsWithdraw_toNumber_facts lv shares payout waive pn hc hprice hnum
  have hpn_floor : pn.mantissa_ ≠ 0 → (10 : ℚ) ^ (-81 : ℤ) ≤ |pn.toRat| := fun hm => by
    ...
    exact STAmount.canonical_disj_abs_toRat_ge payout
      (LawfulVault.sharesToAssetsWithdraw_disj_canonical lv shares payout waive hc hprice hmv) hmv
...
fields, leaving `lossUnrealized` untouched, so it cancels in
the difference. -/
theorem LawfulVault.withdraw_withdrawNav_change (lv : LawfulVault) (amount : WithdrawAmount)
  ...
  · rw [hr]
    simp only [RawVault.withdrawNav, RawVault.toExact]; ring

/-- Proof body of `LawfulVault.withdraw_no_dilution`. Under `lossUnrealized = 0`
(so `withdrawNav = assetsTotal`), the exact-final exit zeroes the
...
theorem LawfulVault.withdraw_no_dilution_proof (lv : LawfulVault) (amount : WithdrawAmount) (r : WithdrawResult)
    ...
    (hSfit : (lv.toExact.sharesTotal : ℚ) ≤ 2 ^ 63 - 1)
    (hok : lv.withdraw amount false = .ok r) (herr : r.error = none) :
    r.vault'.withdrawNav * (lv.toExact.sharesTotal : ℚ) ≥
      lv.withdrawNav * (r.vault'.toExact.sharesTotal : ℚ) * (1 - depositε) := by
  set A : ℚ := lv.toExact.assetsTotal with hA_def
  set S : ℚ := (lv.toExact.sharesTotal : ℚ) with hS_def
  have hAnn : 0 ≤ A := lv.exact.assetsTotal_nonneg
  have hl0 : lv.lossUnrealized.toRat = 0 := hL
  have hNav_v : lv.withdrawNav = A := by
    unfold RawVault.withdrawNav; rw [hL]; ring
  -- reduce the run
  obtain ⟨cw, aN, sta, hcomp, herr2, han, hlt, hsta, hsb, hdisj⟩ :=
    LawfulVault.withdraw_success_reduces lv amount false r hok herr
  rcases hdisj with ⟨-, -, allAvail, -, hr⟩ |
  ...
  · -- exact-final exit: `sharesTotal' = 0`, both sides are `0`
    ...
    have hNav_r : r.vault'.withdrawNav = 0 := by
      rw [hr.2]; simp only [RawVault.withdrawNav, RawVault.toExact, Number.toRat_zero, hl0]; ring
  ...
  · -- non-final exit: the composition core
    ...
    have hprice : lv.sharesToAssetsWithdraw r.sharesBurned false = .ok r.assets' :=
      LawfulVault.withdraw_payout_priced lv amount false sta r hok herr hsta hfin
    obtain ⟨hp_nn, hideal_nn, hp_up, -⟩ :=
      LawfulVault.sharesToAssetsWithdraw_spec_raw lv r.sharesBurned r.assets' false hnn hc hnav hprice
    set x : ℚ := r.sharesBurned.toRat with hx_def
    set p : ℚ := r.assets'.toRat with hp_def
    have hideal_eq : lv.idealAssetsWithdraw false x = A * x / S := by
    ...
    have hra : r.assets' = cw.assets' := hr.1
    have hnum_r : r.assets'.toNumber .to_nearest = .ok aN := by rw [hra]; exact han
    obtain ⟨haN_val, haN_norm⟩ :=
      LawfulVault.sharesToAssetsWithdraw_toNumber_facts lv r.sharesBurned r.assets' false aN hc hprice hnum_r
    have hle_AA : p ≤ lv.assetsAvailable.toRat := by
      have hbridge := operator_lt_iff lv.assetsAvailable aN lv.wf.assetsAvailable_norm haN_norm
      by_contra hcp; push_neg at hcp
      have : lv.assetsAvailable.operator_lt aN = true := by rw [hbridge, haN_val]; exact hcp
      rw [this] at hlt; exact absurd hlt (by simp)
    have hle_AT : p ≤ lv.assetsTotal.toRat := le_trans hle_AA lv.exact.assetsAvailable_le
    have hApm : 0 ≤ A - p := by have : p ≤ A := hle_AT; linarith
    -- raw-stage update lower bound on the stored `assetsTotal`
    have hround : RoundsWithin at' (lv.assetsTotal.toRat - r.assets'.toRat) .to_nearest
        (6 / (2 ^ 63 - 3 : ℚ)) :=
      LawfulVault.sharesToAssetsWithdraw_sub_stored_raw lv r.sharesBurned r.assets' false aN
        lv.assetsTotal at' hc hprice hnum_r hp_nn lv.wf.assetsTotal_norm
        lv.exact.assetsTotal_nonneg hle_AT hat
    ...
    have hSpos : 0 < S := by
      have hAAp : 0 < lv.assetsAvailable.toRat := lt_of_lt_of_le hp_pos hle_AA
      have hApos : 0 < A := lt_of_lt_of_le hAAp lv.exact.assetsAvailable_le
      rcases eq_or_lt_of_le (show (0 : ℚ) ≤ S by rw [hS_def]; positivity) with hS0 | hSp
      · exfalso
        have hSnat : lv.toExact.sharesTotal = 0 := by
          have : ((lv.toExact.sharesTotal : ℕ) : ℚ) = 0 := by rw [← hS_def]; exact hS0.symm
          exact_mod_cast this
        exact absurd (lv.exact.empty_shares hSnat).1 (ne_of_gt hApos)
      · exact hSp
    ...
    have hNav_r : r.vault'.withdrawNav = at'.toRat := by
      rw [hr.2]; simp only [RawVault.withdrawNav, RawVault.toExact, hl0]; ring
    ...
    have hSharesEq : (r.vault'.toExact.sharesTotal : ℚ) = S - x := by
      have hval := (LawfulVault.withdraw_vault_updates_proof lv amount false sta r hp_nn hnn hc hSnt
...
`assetsTotal`: it rewrites only the three balance fields, leaving `lossUnrealized`
untouched. -/
theorem LawfulVault.clawback_withdrawNav_change (lv : LawfulVault) (assets holderShares : STAmount)
    (r : ClawbackResult)
...
/-- Proof body of `LawfulVault.clawback_no_dilution`. Clawback prices its recovery with the
...
side conditions. -/
theorem LawfulVault.clawback_no_dilution_proof (lv : LawfulVault) (assets holderShares : STAmount) (r : ClawbackResult)
    (hL : lv.toExact.lossUnrealized = 0)
    (hnav : lv.WithdrawNavExact false)
    (hc : assets.Canonical)
  ...
  have hAnn : 0 ≤ A := lv.exact.assetsTotal_nonneg
  have hl0 : lv.lossUnrealized.toRat = 0 := hL
  have hNav_v : lv.withdrawNav = A := by unfold RawVault.withdrawNav; rw [hL]; ring
  ...
  have hle_AT : p ≤ lv.assetsTotal.toRat := le_trans hle_AA lv.exact.assetsAvailable_le
  have hApm : 0 ≤ A - p := by have : p ≤ A := hle_AT; linarith
  have hround : RoundsWithin at' (lv.assetsTotal.toRat - r.assetsRecovered.toRat) .to_nearest
      (6 / (2 ^ 63 - 3 : ℚ)) :=
    LawfulVault.sharesToAssetsWithdraw_sub_stored_raw lv r.sharesDestroyed r.assetsRecovered false arn
      lv.assetsTotal at' hcanon_sd hprice hnum_r hp_nn lv.wf.assetsTotal_norm
      lv.exact.assetsTotal_nonneg hle_AT hat
  have hA'_lo : (A - p) * (1 - 6 / (2 ^ 63 - 3 : ℚ)) ≤ at'.toRat := by
    have h : |at'.toRat - (A - p)| ≤ |A - p| * (6 / (2 ^ 63 - 3 : ℚ)) := hround
    rw [abs_of_nonneg hApm] at h
  ...
  have hcore : A * (S - x) * (1 - depositε) ≤ at'.toRat * S :=
    withdraw_no_dilution_arith A S x p at'.toRat (12 / (2 ^ 63 - 3)) (6 / (2 ^ 63 - 3))
      ...
      (by norm_num) (by rw [depositε_eq]; norm_num)
  have hNav_r : r.vault'.withdrawNav = at'.toRat := by
    rw [hr]; simp only [RawVault.withdrawNav, RawVault.toExact, hl0]; ring
  have hle_x : x ≤ S := by rw [hx_def]; linarith [hmargin, hSpos]
  have hSharesEq : (r.vault'.toExact.sharesTotal : ℚ) = S - x :=
    (LawfulVault.clawback_vault_updates_proof' lv assets holderShares r hnav hc hSic hSc hSnn
      hok herr).2.2 ⟨hle_x, hSfit⟩
  rw [ge_iff_le, hNav_v, hNav_r, hSharesEq]
  exact hcore

private lemma navEq (lw : LawfulVault)
    (hL : lw.toExact.lossUnrealized = 0) : lw.withdrawNav = lw.toExact.assetsTotal := by
  unfold RawVault.withdrawNav; rw [hL]; ring
...
The two base hypotheses (`loss = 0` and asset parity) are an induction invariant (every modeled
...
`burnShares` share-only decrease, carrying `Lawful`, `loss = 0`, and parity forward by
the field preservation the success reductions expose. -/
theorem LawfulVault.ReachableFromIn.no_dilution_proof (lv : LawfulVault) (n : ℕ) (v : LawfulVault)
    ...
    v.withdrawNav * (lv.toExact.sharesTotal : ℚ) ≥
      lv.withdrawNav * (v.toExact.sharesTotal : ℚ) * (1 - depositε) ^ n := by
  -- the ops return `LawfulVault`, so each reachable `u` is lawful by construction
  suffices H : ∀ (u : LawfulVault) (m : ℕ), LawfulVault.ReachableFromIn lv u m →
      u.toExact.lossUnrealized = 0 ∧
      u.assetsAvailable = u.assetsTotal ∧
      (0 < (u.toExact.sharesTotal : ℚ) ∨ lv.withdrawNav = 0) ∧
      u.withdrawNav * (lv.toExact.sharesTotal : ℚ) ≥
        lv.withdrawNav * (u.toExact.sharesTotal : ℚ) * (1 - depositε) ^ m by
    exact (H v n h).2.2.2
  intro u m hum
  induction hum with
  | refl =>
    refine ⟨hwL, hwAV, ?_, ?_⟩
    · by_cases hSw : (0 : ℚ) < (lv.toExact.sharesTotal : ℚ)
      · exact Or.inl hSw
      · refine Or.inr ?_
        have hSw0 : lv.toExact.sharesTotal = 0 := by
          have hle : (lv.toExact.sharesTotal : ℚ) ≤ 0 := not_lt.mp hSw
          have hge : (0 : ℚ) ≤ (lv.toExact.sharesTotal : ℚ) := by exact_mod_cast Nat.zero_le _
          have : (lv.toExact.sharesTotal : ℚ) = 0 := le_antisymm hle hge
          exact_mod_cast this
        have hA0 : lv.toExact.assetsTotal = 0 := (lv.exact.empty_shares hSw0).1
        unfold RawVault.withdrawNav; rw [hA0, hwL]; ring
    · rw [pow_zero, mul_one]
  | deposit u' k amount isDonation r prev hrun hcanon hpos hcnz hSsz ih =>
    obtain ⟨hLu0, hAVu, hSuw, hratio_u⟩ := ih
    -- the taken amount and issued shares are nonnegative (derived from the run)
    obtain ⟨hDnn, hSnn⟩ :=
      LawfulVault.deposit_result_nonneg u' amount isDonation  hcanon (le_of_lt hpos) r hrun
    ...
    have hSw_nn : (0 : ℚ) ≤ (lv.toExact.sharesTotal : ℚ) := by exact_mod_cast Nat.zero_le _
    have hNu_nn : 0 ≤ u'.withdrawNav := u'.exact.withdraw_nav_nonneg
    have hLeq := LawfulVault.deposit_preserves_unrealized u' amount isDonation r hrun
    ...
    have hAVr : r.vault'.assetsAvailable = r.vault'.assetsTotal :=
      LawfulVault.deposit_asset_parity u' amount isDonation r hAVu hrun
    have hNr_nn : 0 ≤ r.vault'.withdrawNav := r.vault'.exact.withdraw_nav_nonneg
    ...
    have hSle : (u'.toExact.sharesTotal : ℚ) ≤ (r.vault'.toExact.sharesTotal : ℚ) := by
      ...
      · obtain ⟨hveq, -, -⟩ := LawfulVault.deposit_error_unchanged u' amount isDonation r hrun herr
        rw [hveq]
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        have heq := (LawfulVault.deposit_vault_updates u' amount isDonation  hcanon hpos r hrun herr').2.2 hSsz
      ...
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        ...
        · subst hdon
          have hSeq : (r.vault'.toExact.sharesTotal : ℚ) = (u'.toExact.sharesTotal : ℚ) := by
            have := LawfulVault.deposit_donation_sharesTotal_toExact u' amount r hrun herr'
            exact_mod_cast this
          have hNav_u := navEq u' hLu0
          have hNr_eq := navEq r.vault' hLr0
          have hupd := (LawfulVault.deposit_vault_updates u' amount true  hcanon hpos r hrun herr').1
          ...
          have hAnn : 0 ≤ u'.toExact.assetsTotal := u'.exact.assetsTotal_nonneg
        ...
        · have hnd : isDonation = false := by
          ...
          subst hnd
          exact LawfulVault.deposit_no_dilution_proof u' amount r  hcanon hpos hLu0 hcnz hSsz hrun herr'
    refine ⟨hLr0, hAVr, ?_, ?_⟩
      ...
      · rw [pow_succ]
        ...
        have h1 : u'.withdrawNav * (r.vault'.toExact.sharesTotal : ℚ) * (1 - depositε)
            * (lv.toExact.sharesTotal : ℚ)
            ≤ r.vault'.withdrawNav * (u'.toExact.sharesTotal : ℚ) * (lv.toExact.sharesTotal : ℚ) :=
          mul_le_mul_of_nonneg_right hstep hSw_nn
        have h2 : lv.withdrawNav * (u'.toExact.sharesTotal : ℚ) * (1 - depositε) ^ k
            * ((r.vault'.toExact.sharesTotal : ℚ) * (1 - depositε))
            ≤ u'.withdrawNav * (lv.toExact.sharesTotal : ℚ)
              * ((r.vault'.toExact.sharesTotal : ℚ) * (1 - depositε)) :=
          mul_le_mul_of_nonneg_right hratio_u hey_nn
        have key : (lv.withdrawNav * (r.vault'.toExact.sharesTotal : ℚ)
  ...
  | withdraw u' k amount r prev hrun hnav hcnz hSc hcanon hSnt hSneg hmargin hSfit ih =>
    ...
    have hDnn : 0 ≤ r.assets'.toRat :=
      LawfulVault.withdraw_assets_nonneg u' amount false  hLu0 r hrun hSc hSnt hSneg
    have hsle : r.sharesBurned.toRat ≤ (u'.toExact.sharesTotal : ℚ) := by linarith [hmargin, hSu_nn]
    have hLeq := LawfulVault.withdraw_preserves_unrealized u' amount false r hrun
    have hLr0 : r.vault'.toExact.lossUnrealized = 0 := by
      show r.vault'.lossUnrealized.toRat = 0; rw [hLeq]; exact hLu0
    have hAVr : r.vault'.assetsAvailable = r.vault'.assetsTotal :=
      LawfulVault.withdraw_asset_parity u' amount false r hAVu hrun
    have hNr_nn : 0 ≤ r.vault'.withdrawNav := r.vault'.exact.withdraw_nav_nonneg
    have hSr_nn : (0 : ℚ) ≤ (r.vault'.toExact.sharesTotal : ℚ) := by exact_mod_cast Nat.zero_le _
    have hstep : r.vault'.withdrawNav * (u'.toExact.sharesTotal : ℚ) ≥
        u'.withdrawNav * (r.vault'.toExact.sharesTotal : ℚ) * (1 - depositε) := by
      by_cases herr : r.error.isSome = true
      · obtain ⟨hveq, -, -⟩ := LawfulVault.withdraw_error_unchanged u' amount false r hrun herr
        rw [hveq]; nlinarith [mul_nonneg hNu_nn hSu_nn, he_le1]
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        exact LawfulVault.withdraw_no_dilution_proof u' amount r  hLu0 hnav hcnz hnn hcanon hSnt hmargin hSfit hrun herr'
    ...
    have hSrpos : 0 < (u'.toExact.sharesTotal : ℚ) → 0 < (r.vault'.toExact.sharesTotal : ℚ) := by
      ...
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        obtain ⟨cw, aN', sta, hcomp, herr2, han, hlt, hst, hsb, hdisj⟩ :=
          LawfulVault.withdraw_success_reduces u' amount false r hrun herr'
        have hSfit' : u'.sharesTotal.toRat ≤ 2 ^ 63 - 1 := by
          have := hSfit; rwa [RawVault.WF.toExact_sharesTotal u'.toRawVault u'.wf] at this
        obtain ⟨hsta_nt, hsta_off, hsta_neg, hsta_mv, hsta_val⟩ :=
          STAmount.ofNumber_int64_shape u'.sharesTotal .to_nearest sta
          u'.wf.sharesTotal_norm u'.wf.sharesTotal_nonneg u'.wf.sharesTotal_int hSfit' hst
        have hstaS : sta.toRat = (u'.toExact.sharesTotal : ℚ) := by
          rw [hsta_val, RawVault.WF.toExact_sharesTotal u'.toRawVault u'.wf]
        ...
        · exfalso
          ...
          linarith [hSu_pos]
        · have hupd := (LawfulVault.withdraw_vault_updates_proof u' amount false  sta r hDnn hnn hcanon hSnt
          ...
          have hSrq : (r.vault'.toExact.sharesTotal : ℚ)
            ...
            have hcast : ((r.vault'.toExact.sharesTotal : ℕ) : ℚ) = r.vault'.sharesTotal.toRat :=
              RawVault.WF.toExact_sharesTotal r.vault'.toRawVault r.vault'.wf
  ...
  | clawback u' k assets holderShares r prev hrun hnav hcanon hSic hSc hSnn hmargin hSfit ih =>
    ...
    have hsuc_pos : r.error = none → 0 < (u'.toExact.sharesTotal : ℚ) := by
      ...
      obtain ⟨hnn_sd, hcanon_sd, -, -, hsdnz⟩ :=
        LawfulVault.clawback_recovery_priced' u' assets holderShares r  hnav hcanon hSc hSnn
          hrun herr'
      have hxpos : 0 < r.sharesDestroyed.toRat :=
        STAmount.Canonical.toRat_pos_of_nonneg r.sharesDestroyed hcanon_sd hnn_sd hsdnz
      linarith [hmargin, hxpos]
    have hLeq := LawfulVault.clawback_preserves_unrealized u' assets holderShares r hrun
    have hLr0 : r.vault'.toExact.lossUnrealized = 0 := by
      show r.vault'.lossUnrealized.toRat = 0; rw [hLeq]; exact hLu0
    have hAVr : r.vault'.assetsAvailable = r.vault'.assetsTotal :=
      LawfulVault.clawback_asset_parity u' assets holderShares r hAVu hrun
    have hNr_nn : 0 ≤ r.vault'.withdrawNav := r.vault'.exact.withdraw_nav_nonneg
    have hSr_nn : (0 : ℚ) ≤ (r.vault'.toExact.sharesTotal : ℚ) := by exact_mod_cast Nat.zero_le _
    have hstep : r.vault'.withdrawNav * (u'.toExact.sharesTotal : ℚ) ≥
        u'.withdrawNav * (r.vault'.toExact.sharesTotal : ℚ) * (1 - depositε) := by
      by_cases herr : r.error.isSome = true
      · obtain ⟨hveq, -, -⟩ := LawfulVault.clawback_error_unchanged u' assets holderShares r hrun herr
        rw [hveq]; nlinarith [mul_nonneg hNu_nn hSu_nn, he_le1]
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        exact LawfulVault.clawback_no_dilution_proof u' assets holderShares r  hLu0 hnav hcanon
    ...
    have hSrpos : 0 < (u'.toExact.sharesTotal : ℚ) → 0 < (r.vault'.toExact.sharesTotal : ℚ) := by
      ...
      · have herr' : r.error = none := Option.not_isSome_iff_eq_none.mp herr
        ...
        have hSrq := (LawfulVault.clawback_vault_updates_proof' u' assets holderShares r  hnav hcanon
  ...
  | burnShares u' k sharesDestroyed sharesTotalAmount u'' prev hcan hcanon hSneg hle hnn hden hSfit hburn ih =>
    ...
    have hguard : (u'.sharesTotal.mantissa_ == 0 ||
      ...
      simp only [LawfulVault.canBurnShares] at hcan
    ...
    have hLeq := LawfulVault.burnShares_preserves_unrealized u' sharesDestroyed u'' hburn
    ...
    have hAVr : u''.assetsAvailable = u''.assetsTotal :=
      LawfulVault.burnShares_asset_parity u' sharesDestroyed u'' hAVu hburn
    have hNr_nn : 0 ≤ u''.withdrawNav := u''.exact.withdraw_nav_nonneg
    have hSw_nn : (0 : ℚ) ≤ (lv.toExact.sharesTotal : ℚ) := by exact_mod_cast Nat.zero_le _
    refine ⟨hLr0, hAVr, Or.inr hNw0, ?_⟩
