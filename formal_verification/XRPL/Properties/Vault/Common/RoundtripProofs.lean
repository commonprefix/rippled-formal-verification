...
/-! # Proof support for `LawfulVault.deposit_withdraw_roundtrip` -/
...
`roundtrip_algebra`. -/
private lemma roundtrip_charge_Q (lv : LawfulVault)
    (shares c : STAmount)
    ...
    ∃ Q : Number,
      lv.idealChargeDeposit shares.toRat * (1 - 19 / (2 ^ 63 - 3)) ≤ Q.toRat ∧
      Q.toRat ≤ lv.idealChargeDeposit shares.toRat * (1 + 19 / (2 ^ 63 - 3)) ∧
      Q.toRat ≤ c.toRat ∧
      c.toRat ≤ Q.toRat + (10 : ℚ) ^ c.exponent ∧
      0 < lv.idealChargeDeposit shares.toRat ∧
      lv.idealChargeDeposit shares.toRat =
  ...
  obtain ⟨Q, hcQ, hidpos, hQnz, hQz⟩ :=
    sharesToAssetsDeposit_charge_nonempty_raw lv shares c hshc hshnt hshpos hmz hsad
  ...
  have hQm : Q.mantissa_ ≠ 0 := by
    ...
    · exact STAmount.ofNumber_integral_source_ne_zero lv.numericType Q .upward c hint hcQ hc0
    · have hfrac : lv.numericType = .fractional := by
      ...
      exact STAmount.ofNumber_iou_mantissa_ne_zero lv.numericType Q .upward c hfrac hcQ hc0
  obtain ⟨hQnorm, hQneg, hband⟩ := hQnz hQm
  have hbandle := abs_le.mp hband
  refine ⟨Q, ?_, ?_, ?_, ?_, hidpos, hideal_eq⟩
  · nlinarith [hbandle.1]
  · nlinarith [hbandle.2]
  · exact STAmount.ofNumber_upward_ge lv.numericType Q c hQnorm hQneg hcQ hc0
  · -- upward `ofNumber` ceiling, single ULP
    ...
    · obtain ⟨hr_lo, hr_hi⟩ := hQnorm.mantissaBounds_nat hQm
      ...
      exact STAmount.ofNumber_upward_ceiling_bounds lv.numericType Q c hfrac
...
/-- **Proof body of `LawfulVault.deposit_withdraw_roundtrip`.** Wires the banked raw
...
`12/(2^63-3)`) with its downward floor. `r₁.vault'` is a `LawfulVault` from
...
theorem LawfulVault.deposit_withdraw_roundtrip_proof (lv : LawfulVault) (amountDeposit : STAmount)
  ...
  obtain ⟨am, aD, sC, cN, sN, at', av', st', hround, hamz, _hshdon, hinsolv, _hdon,
    ...
    LawfulVault.deposit_success_reduces lv amountDeposit false _ hok₁ herr₁
  ...
  set V' : RawVault := { lv with assetsTotal := at', assetsAvailable := av', sharesTotal := st' } with hV'def
  ...
  have hRW : rlv = (⟨V', hWF₁, hrlv ▸ rlv.valid⟩ : LawfulVault) := by
    obtain ⟨rr, rwf, rval⟩ := rlv
    obtain rfl : rr = V' := hrlv
    rfl
  have hinsolv' : lv.isInsolvent = false := hinsolv rfl
  obtain ⟨shares, hats, hshz, hsad, _hgt, hseq⟩ := computeDeposit_success_reduces lv am aD sC (hcomp rfl)
  obtain ⟨hshc, hshnt⟩ := assetsToSharesDeposit_int64_canonical lv am shares hats
  have hamCanon : am.Canonical := by
    rcases roundToVaultExponent_canonical_or_isZero amountDeposit am lv.assetsTotal hcanon hround with hc | hz
    · exact hc
  ...
  have hshpos : 0 < shares.toRat := assetsToSharesDeposit_pos lv am shares hamCanon ham_pos hats hshz
  have hSCpos : 0 < sC.toRat := by rw [hseq]; exact hshpos
  have hsad' : sharesToAssetsDeposit lv sC = .ok aD := by rw [hseq]; exact hsad
  have hshc' : sC.IntegralCanonical := hseq ▸ hshc
  have hshnt' : sC.mNumericType = .int64 := hseq ▸ hshnt
  obtain ⟨hcN_val, hcN_norm⟩ := sharesToAssetsDeposit_toNumber_exact lv sC aD cN hshc' hshnt' hsad' hcN
  have hcN_nn : 0 ≤ cN.toRat := by rw [hcN_val]; exact hDnn
  have hL0 : lv.lossUnrealized = Number.zero := by
    have hmzL : lv.lossUnrealized.mantissa_ = 0 := by
      by_contra h; exact (Number.toRat_ne_zero_of_mantissa_ne_zero lv.lossUnrealized h) hL
    exact Number.eq_zero_of_mantissa_zero lv.lossUnrealized lv.wf.lossUnrealized_norm hmzL
  have hNav₁ : V'.WithdrawNavExact false := by
    refine ⟨at', ?_, ?_⟩
    · show at'.operator_sub lv.lossUnrealized .to_nearest = .ok at'
      rw [hL0]; exact operator_sub_zero_right _ _
    · show at'.toRat = at'.toRat - lv.lossUnrealized.toRat
      rw [show lv.lossUnrealized.toRat = 0 from hL]; ring
  obtain ⟨cw, aN', sta, hcomp2, herr_cw, haN', hlt, hsta, hsb, hdisj⟩ :=
    LawfulVault.withdraw_success_reduces rlv (.vaultShares sC) false r₂ hok₂' herr₂
  ...
  have hpay := LawfulVault.sharesToAssetsWithdraw_spec_raw ⟨V', hWF₁, hrlv ▸ rlv.valid⟩ sC cw.assets' false hSCpos.le hSC_canon hNav₁ hstw
  have hAnn : 0 ≤ lv.assetsTotal.toRat := lv.exact.assetsTotal_nonneg
  have hru := operator_add_nonneg_rounds lv.assetsTotal cN at' lv.wf.assetsTotal_norm hcN_norm hAnn hcN_nn hat
  have hTnn : 0 ≤ lv.assetsTotal.toRat + cN.toRat := add_nonneg hAnn hcN_nn
  simp only [RoundsWithin, RatValued.toRat] at hru
  rw [abs_of_nonneg hTnn] at hru
  have hstore := abs_le.mp hru
  have hSharesEq : (V'.toExact.sharesTotal : ℚ) = (lv.toExact.sharesTotal : ℚ) + sC.toRat := by
    rw [← hrlv]
    exact (LawfulVault.deposit_vault_updates lv amountDeposit false hcanon hpos
      ⟨none, rlv, aD, sC⟩ hok₁ herr₁).2.2 hSsz
  have hAT_ex : lv.toExact.assetsTotal = lv.assetsTotal.toRat := rfl
  have hSTvnn : (0:ℚ) ≤ (lv.toExact.sharesTotal : ℚ) := by positivity
  have hV'ST : V'.sharesTotal.toRat = (lv.toExact.sharesTotal : ℚ) + sC.toRat := by
    rw [← RawVault.WF.toExact_sharesTotal V' hWF₁, hSharesEq]
  have hIA_eq : V'.idealAssetsWithdraw false sC.toRat
      = at'.toRat * sC.toRat / ((lv.toExact.sharesTotal : ℚ) + sC.toRat) := by
    ...
    have hwn : V'.toExact.assetsTotal - V'.toExact.lossUnrealized
        = at'.toRat := by
      show at'.toRat - lv.lossUnrealized.toRat = at'.toRat
      rw [show lv.lossUnrealized.toRat = 0 from hL]; ring
    rw [hwn]
  ...
  · -- FINAL branch: the empty starting vault (`sharesTotal = 0`), where redeeming the
    ...
    -- `av' = cN = amountDeposit'.toNumber` and the payout `ofNumber lv.numericType av'`
    ...
    have hSeq : sC.toRat = V'.sharesTotal.toRat :=
      (LawfulVault.operator_eq_total_iff ⟨V', hWF₁, hrlv ▸ rlv.valid⟩ sta sC hSCpos hSC_canon hshnt' hsta).mp
        (by rw [← hsr]; exact hfin)
    have hSTv0 : (lv.toExact.sharesTotal : ℚ) = 0 := by rw [hV'ST] at hSeq; linarith
    have hSTvN : lv.toExact.sharesTotal = 0 := by exact_mod_cast hSTv0
    obtain ⟨_, hAA0⟩ := lv.exact.empty_shares hSTvN
    have hAA0' : lv.assetsAvailable.toRat = 0 := hAA0
    have hAAm0 : lv.assetsAvailable.mantissa_ = 0 := by
      by_contra h
      exact (Number.toRat_ne_zero_of_mantissa_ne_zero lv.assetsAvailable h) hAA0'
    have hAAzero : lv.assetsAvailable = Number.zero :=
      Number.eq_zero_of_mantissa_zero lv.assetsAvailable lv.wf.assetsAvailable_norm hAAm0
    rw [hAAzero] at hav
    have hV'aa : V'.assetsAvailable = av' := by rw [hV'def]
    have hV'nt : V'.numericType = lv.numericType := by rw [hV'def]
    rw [hV'nt, hV'aa] at hallAvail
    have hnt_aD : aD.mNumericType = lv.numericType :=
      sharesToAssetsDeposit_mNumericType lv sC aD hsad'
    rw [hr2a]
    ...
    · -- nonzero payout: exact value roundtrip `A = C`
      have hcNnz : cN.operator_eq Number.zero = false := by
        ...
        have hsrc := STAmount.ofNumber_source_ne_zero lv.numericType av' .to_nearest allAvail
          hallAvail hAz
      ...
      have hkey : allAvail.toRat = aD.toRat :=
        ofNumber_charge_roundtrip lv.numericType aD cN allAvail hnt_aD hDc hDnn' hcN hallAvail hAz
      refine ⟨?_, ?_⟩
      · rw [hkey]
  ...
  · have hr2a : r₂.assets' = cw.assets' := by rw [hrnf.1]
    ...
    have hStvpos : 0 < (lv.toExact.sharesTotal : ℚ) := by
      have hiff := LawfulVault.operator_eq_total_iff ⟨V', hWF₁, hrlv ▸ rlv.valid⟩ sta sC hSCpos hSC_canon hshnt' hsta
      ...
      rw [hV'ST] at hne_val
      have hSTne : (lv.toExact.sharesTotal : ℚ) ≠ 0 := fun h0 => hne_val (by rw [h0]; ring)
    ...
    · have hAnz : cw.assets'.isZero = false := by
      ...
      have hmz : lv.assetsTotal.mantissa_ ≠ 0 := by
        have hApos : 0 < lv.assetsTotal.toRat := by
          by_contra h; push_neg at h
          have hA0 : lv.assetsTotal.toRat = 0 := le_antisymm h hAnn
          have hins : lv.isInsolvent = true := (LawfulVault.isInsolvent_iff_proof ⟨lv.toRawVault, lv.wf, (RawVault.valid_iff_exact lv.toRawVault lv.wf).mpr lv.exact⟩).mpr
      ...
      have hCnz : aD.isZero = false := by
        ...
        have hApos : 0 < lv.toExact.assetsTotal :=
          lt_of_le_of_ne hAnn (Ne.symm (Number.toRat_ne_zero_of_mantissa_ne_zero lv.assetsTotal hmz))
        obtain ⟨Q, hcQ, hidpos, hQnz, hQz⟩ :=
          sharesToAssetsDeposit_charge_nonempty_raw lv sC aD hshc' hshnt' hSCpos hmz hsad'
        have hIC_eq' : lv.idealChargeDeposit sC.toRat
            = lv.toExact.assetsTotal * sC.toRat / (lv.toExact.sharesTotal : ℚ) := by
          unfold RawVault.idealChargeDeposit RawVault.depositNav
          rw [if_neg (ne_of_gt hApos)]
        ...
        have hA_lo : (10 : ℚ) ^ (-81 : ℤ) ≤ cw.assets'.toRat := by
          have hcanon := LawfulVault.sharesToAssetsWithdraw_disj_canonical ⟨V', hWF₁, hrlv ▸ rlv.valid⟩ sC cw.assets' false hSC_canon hstw hmvne
        ...
        have hat'_eq : at' = lv.assetsTotal := by
          have hh := hat; rw [hcN0, operator_add_zero_right] at hh; exact (Except.ok.inj hh).symm
        have hat'_toRat : at'.toRat = lv.toExact.assetsTotal := by rw [hAT_ex, hat'_eq]
        by_cases hQm : Q.mantissa_ = 0
        · have hICsmall := hQz hQm
          have hIA_le_IC : V'.idealAssetsWithdraw false sC.toRat ≤ lv.idealChargeDeposit sC.toRat := by
          ...
          have h2 : V'.idealAssetsWithdraw false sC.toRat * (1 + 12 / (2 ^ 63 - 3))
              ≤ lv.idealChargeDeposit sC.toRat * (1 + 12 / (2 ^ 63 - 3)) :=
          ...
          have hA_small : cw.assets'.toRat < (10 : ℚ) ^ (-81 : ℤ) := by
            have hlt : lv.idealChargeDeposit sC.toRat * (1 + 12 / (2 ^ 63 - 3)) < 16 * 10 ^ (-32750 : ℤ) := by
        ...
        · by_cases hint : lv.numericType.isIntegral = true
          · obtain ⟨hQnorm, hQneg, _⟩ := hQnz hQm
            have hge := STAmount.ofNumber_integral_upward_ge lv.numericType Q aD hint hQnorm hQneg hcQ
            ...
            linarith [hge, hC0]
          · have hnt : aD.mNumericType = lv.numericType :=
              STAmount.ofNumber_mNumericType lv.numericType Q .upward aD hcQ
            have hfrac : lv.numericType.isIntegral = false := by
              cases h : lv.numericType.isIntegral with | true => exact absurd h hint | false => rfl
      ...
      obtain ⟨Q, hQlo, hQhi, hQClo, hQChi, hICpos, hIC_eq⟩ :=
        roundtrip_charge_Q lv sC aD hshc' hshnt' hSCpos hmz hCnz hsad'
      have hicid : lv.toExact.assetsTotal * sC.toRat
          = lv.idealChargeDeposit sC.toRat * (lv.toExact.sharesTotal : ℚ) := by
        rw [hIC_eq]; field_simp
      have hIAid : V'.idealAssetsWithdraw false sC.toRat
          * ((lv.toExact.sharesTotal : ℚ) + sC.toRat) = at'.toRat * sC.toRat := by
        rw [hIA_eq, div_mul_cancel₀ _ (add_pos hStvpos hSCpos).ne']
      have halg := roundtrip_algebra aD.toRat cw.assets'.toRat (lv.idealChargeDeposit sC.toRat)
        Q.toRat at'.toRat (V'.idealAssetsWithdraw false sC.toRat) lv.toExact.assetsTotal
        sC.toRat (lv.toExact.sharesTotal : ℚ) ((10:ℚ)^cw.assets'.exponent) ((10:ℚ)^aD.exponent)
...
/-! ## Witness for `LawfulVault.deposit_withdraw_roundtrip_attained`
...
/-- The witness vault as a `LawfulVault`. `.toRawVault` is defeq `wvF` by construction,
so the witness deposit/withdraw runs stay computable under `native_decide`. -/
private def wvF_lawful' : LawfulVault := ⟨wvF, wvF_WF, wvF_Valid⟩

set_option linter.style.nativeDecide false in
/-- **Proof body of `LawfulVault.deposit_withdraw_roundtrip_attained`.** Instantiates the
...
theorem LawfulVault.deposit_withdraw_roundtrip_attained_proof :
