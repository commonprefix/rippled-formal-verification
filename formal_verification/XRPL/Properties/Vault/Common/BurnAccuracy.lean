...
theorem Vault.burnShares_sharesTotal_exact_proof (v : Vault) (sharesDestroyed : STAmount)
  ...
  have hv' : vault' = { v with sharesTotal := st' } :=
    (Except.ok.inj (show Except.ok ({ v with sharesTotal := st' } : Vault)
      = Except.ok vault' from hok)).symm
  -- the subtraction is exact
  have hxq := rat_eq_num_cast_of_den_one v.sharesTotal.toRat hv.wf.sharesTotal_int
  ...
  obtain ⟨hst_val, hst_den⟩ := operator_sub_exact_int v.sharesTotal sdn st'
    hv.wf.sharesTotal_norm hsdn_norm hv.wf.sharesTotal_int
