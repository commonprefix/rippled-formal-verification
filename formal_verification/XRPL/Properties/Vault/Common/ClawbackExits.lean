...
runs without a throw, and `LawfulVault.clawback_error_codes_proof` walks every exit
of `LawfulVault.clawback`. The walks use `bind_ok_peel`, `if`-elimination, and the
...
theorem computeClawback_codes (lv : LawfulVault) (assets holderShares : STAmount)
    (cc : ComputeClawbackResult)
    (hok : computeClawback lv assets holderShares = .ok cc) :
  ...
  · rw [if_neg hneg] at hok
    ...
    have handler_path_dry : ∀ (e : Error),
        tryCatch (Except.error e : Except Error (DoResultPR ComputeClawbackResult ComputeClawbackResult PUnit))
          (fun e => if isOverflow e = true then
              pure (DoResultPR.return (⟨some TER.tecPATH_DRY, STAmount.zero lv.numericType,
    ...
    cases hats : assetsToSharesClawback lv assets holderShares with
    | error e =>
      rw [hats, err_bind] at htc
      exact .inr (.inr (handler_path_dry e htc))
    | ok sd =>
      simp only [hats, ok_bind, epure] at htc
      cases hsa : lv.sharesToAssetsWithdraw sd false with
      | error e2 =>
      ...
      | ok ar =>
        ...
        | ok arn =>
          ...
          · rw [if_pos hgt] at htc
            cases hof : STAmount.ofNumber lv.numericType lv.assetsAvailable .to_nearest with
            | error e4 =>
              rw [hof, err_bind] at htc
              exact .inr (.inr (handler_path_dry e4 htc))
            | ok ar2 =>
              simp only [hof, ok_bind] at htc
              cases hats2 : assetsToSharesWithdraw lv ar2 true false with
              | error e5 =>
                rw [hats2, err_bind] at htc
                exact .inr (.inr (handler_path_dry e5 htc))
              | ok sd2 =>
                simp only [hats2, ok_bind] at htc
                cases hsa2 : lv.sharesToAssetsWithdraw sd2 false with
                | error e6 =>
                ...
                | ok ar3 =>
                  ...
                  | ok arn3 =>
                    ...
                    by_cases hgt2 : arn3.operator_gt lv.assetsAvailable = true
                    · rw [if_pos hgt2, tryCatch_ok] at htc
                      rw [← Except.ok.inj htc] at hK
                      have hK2 : Except.ok (⟨some TER.tecINTERNAL, STAmount.zero lv.numericType,
                          STAmount.zero NumericType.int64⟩ : ComputeClawbackResult) = Except.ok cc := hK
                      rw [← Except.ok.inj hK2]
                      exact .inr (.inl rfl)
...
theorem LawfulVault.clawback_error_codes_proof (lv : LawfulVault) (assets holderShares : STAmount)
    (r : ClawbackResult)
  ...
  have hcodes := computeClawback_codes lv assets holderShares result hres
...
theorem LawfulVault.clawback_negative_amount_proof (lv : LawfulVault) (assets holderShares : STAmount)
    (hneg : assets.negative = true) :
    lv.clawback assets holderShares = .ok (.rejected lv .tecINTERNAL) := by
  have hcc : computeClawback lv assets holderShares =
      .ok ⟨some .tecINTERNAL, STAmount.zero lv.numericType, STAmount.zero .int64⟩ := by
...
theorem LawfulVault.clawback_zero_shares_proof (lv : LawfulVault) (assets holderShares : STAmount)
    ...
    lv.clawback assets holderShares = .ok (.rejected lv .tecPRECISION_LOSS) := by
  unfold LawfulVault.clawback
  simp only []
  rw [hcomp, ok_bind]
...
theorem LawfulVault.clawback_recovery_too_small_proof (lv : LawfulVault) (assets holderShares : STAmount)
    (result : ComputeClawbackResult)
