                ...
                | ok ar3 =>
                  ...
                  | ok arn3 =>
                    ...
                    · rw [if_pos hgt2, tryCatch_ok] at htc
                      ...
                      rw [← Except.ok.inj hK2]
                      exact .inr (.inl rfl)
                    · rw [if_neg hgt2] at htc
                      cases hcl : clampToSumExponent v.assetsTotal ar3.operator_neg with
                      | error e8 =>
                        rw [hcl, err_bind] at htc
                        exact .inr (.inr (.inl (handler_path_dry e8 htc)))
                      | ok clamped =>
                        simp only [hcl, ok_bind] at htc
                        cases hfnp : clamped.isFractionalNonPositive with
                        | error e9 =>
                          rw [hfnp, err_bind] at htc
                          exact .inr (.inr (.inl (handler_path_dry e9 htc)))
                        | ok fnp =>
                          simp only [hfnp, ok_bind] at htc
                          by_cases hf : fnp = true
                          · rw [if_pos hf, tryCatch_ok] at htc
                            rw [← Except.ok.inj htc] at hK
                            have hK2 : Except.ok (⟨some TER.tecPRECISION_LOSS,
                                STAmount.zero v.numericType,
                                STAmount.zero NumericType.int64⟩ : ComputeClawbackResult)
                                = Except.ok cc := hK
                            rw [← Except.ok.inj hK2]
                            exact .inr (.inr (.inr rfl))
                          · rw [if_neg hf, tryCatch_ok] at htc
                            rw [← Except.ok.inj htc] at hK
                            have hK2 : Except.ok (⟨none, clamped, sd2⟩ : ComputeClawbackResult)
                                = Except.ok cc := hK
                            rw [← Except.ok.inj hK2]
                            exact .inl rfl
          · rw [if_neg hgt] at htc
            cases hcl : clampToSumExponent v.assetsTotal ar.operator_neg with
            | error e8 =>
              rw [hcl, err_bind] at htc
              exact .inr (.inr (.inl (handler_path_dry e8 htc)))
            | ok clamped =>
              ...
              | ok fnp =>
                ...
                · rw [if_neg hf, tryCatch_ok] at htc
                  rw [← Except.ok.inj htc] at hK
                  have hK2 : Except.ok (⟨none, clamped, sd⟩ : ComputeClawbackResult)
                      = Except.ok cc := hK
                  rw [← Except.ok.inj hK2]
                  exact .inl rfl
...
theorem Vault.clawback_error_codes_proof (v : Vault) (assets holderShares : STAmount)
  ...
  · rw [if_pos h1] at hok; injection hok with h; rw [← h]
    rcases hcodes with hc | hc | hc | hc
    · exact .inl hc
    · exact .inr (.inl hc)
