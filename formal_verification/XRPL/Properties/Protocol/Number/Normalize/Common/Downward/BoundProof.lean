...
theorem normalize_rounding_bound_downward (n result : Number)
  ...
  · -- ===== CUSP RANGE: maxRep < zm ≤ maxRepUp — no digits were pushed, f = 0 =====
    ...
    have h_ground : g.round .downward = -2 := by
      unfold Guard.round
      rw [if_pos hempty]
    have h_gbool_false :
    ...
    · -- ===== negative n =====
      ...
      · -- v = maxRepNat + 3: either exact (zm = maxRepUp) or the fired clamp.
        subst hv
        ...
        · -- zm = maxRepUp: result = n exactly.
          ...
          refine ⟨le_of_eq h_req, ?_⟩
          rw [h_req, show n.toRat - n.toRat = 0 from by ring]
          exact h_allow_pos
        · -- the fired clamp to maxRepUp: result below the truth, error < 3.
          refine ⟨?_, ?_⟩
          · rw [h_result_signed, h_truth_signed]
            nlinarith [h10ze'_nn, hzm_q_le3]
          · rw [h_abs_neg, h_truth_signed, h_result_signed]
            rw [show -((zm.toNat : ℚ) * 10 ^ ze') - -(((maxRepNat : ℚ) + 3) * 10 ^ ze')
    ...
    · -- ===== non-negative n =====
      ...
      · -- v = maxRepNat: the truncating clamp, below the truth.
        ...
        refine ⟨?_, ?_⟩
        · rw [h_result_signed, h_truth_signed]
          exact mul_le_mul_of_nonneg_right (by linarith) h10ze'_nn
        · rw [h_abs_eq, h_truth_signed, h_result_signed]
          rw [show (zm.toNat : ℚ) * 10 ^ ze' - (maxRepNat : ℚ) * 10 ^ ze'
          ...
          rw [h_denom_val, show (zm.toNat : ℚ) * (10 / (maxRepCuspTarget : ℚ))
              ...
              lt_div_iff₀ (by norm_num : (0 : ℚ) < maxRepCuspTarget)]
          nlinarith [hzm_q_gt, hzm_q_le3]
      · -- v = maxRepNat + 3: the pushed-guard decision is dead for positives,
