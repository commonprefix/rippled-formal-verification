...
-- Discrete-grid rounding identification for `operator_add` under `.downward`.
...
theorem operator_add_no_inbetween_below_downward (x y result : Number)
...
theorem operator_add_rounded_downward_proof (x y result : Number)
    (hx : x.isNormalized) (hy : y.isNormalized)
    (hok : Number.operator_add x y .downward = .ok result)
    (hresult : result.mantissa_ ≠ 0) :
  ...
  have h_truth_ne : x.toRat + y.toRat ≠ 0 :=
    operator_add_truth_ne x y hx hy hx_mant_ne hy_mant_ne heq_guard .downward result hok
  have h_dir : result.toRat ≤ x.toRat + y.toRat :=
    (operator_add_rounds_downward x y result hx hy hx_mant_ne hy_mant_ne heq_guard
      hok hresult).1
  have h_result_norm : result.isNormalized := by
    by_cases h_sign_eq : x.negative_ = y.negative_
    · obtain ⟨_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, h_norm, _, _⟩ :=
        operator_add_algorithmic_facts_same_sign_downward x y result hx hy
          hx_mant_ne hy_mant_ne h_sign_eq heq_guard hok
      exact h_norm
    · exact operator_add_result_isNormalized x y result .downward hx hy
  ...
  have h_bound : |result.toRat - (x.toRat + y.toRat)|
    ...
    le_of_lt (operator_add_rounding_bound_downward_tight x y result hx hy
  ...
  exact closest_lower_of_no_inbetween result (x.toRat + y.toRat) h_result_norm hresult
    ...
    (operator_add_no_inbetween_below_downward x y result hx hy hx_mant_ne hy_mant_ne
