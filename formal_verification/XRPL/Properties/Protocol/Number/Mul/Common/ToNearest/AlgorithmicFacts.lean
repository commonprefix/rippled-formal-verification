...
/-! ## Algorithmic decomposition for `operator_mul`

Packages the `scaleDown128 + doRoundUp + normalize` decomposition into a single
existential, avoiding repeated unfolding in downstream proofs. -/

theorem operator_mul_algorithmic_facts_to_nearest (x y result : Number)
    (hx : x.isNormalized) (hy : y.isNormalized)
    (hx_mant_ne : x.mantissa_ ≠ 0) (hy_mant_ne : y.mantissa_ ≠ 0)
    (hok : Number.operator_mul x y .to_nearest = .ok result)
    (hresult : result.mantissa_ ≠ 0) :
    ∃ (zm : UInt64) (ze' : Int) (f : ℚ) (g : Guard) (res_pos : RoundResult),
      MulFactsToNearest x y result .to_nearest zm ze' f g res_pos := by
  obtain ⟨zn, zm, ze', f, g, res, hzn, hzm_succ, hzm_le_maxRep, hf_nn, hf_lt, hfloor,
      habs, hf_rep, hsbit, h_rup, hok'⟩ :=
    operator_mul_decompose x y result .to_nearest hx hy hx_mant_ne hy_mant_ne hok
  ...
  have h_inv := doRoundUp_output_invariants_to_nearest_upTo_maxRepUp g zn zm ze' hzm_ge hzm_le_maxRep .overflow res h_rup hres_mant_ne
  ...
  have h_rup_pos : g.doRoundUp false zm ze' largeRange.min largeRange.max .to_nearest .overflow = .ok res_pos :=
    doRoundUp_false_from_ok g zn zm ze' .to_nearest .overflow res h_rup
  ...
  have h_res_neg_eq_zn : result.negative_ = zn := by
    rw [h_result_eq_res]
    exact doRoundUp_negative_of_mant_ne g zn zm ze' _ _ _ .overflow res h_rup hres_mant_ne
  exact ⟨zm, ze', f, g, res_pos,
    ⟨hzm_ge, hzm_le_maxRep, hf_nn, hf_lt, hfloor, habs, h_rup_pos, h_result_abs,
     hres_pos_mant_ne, hf_rep, h_res_neg_eq_zn.trans hzn⟩⟩
