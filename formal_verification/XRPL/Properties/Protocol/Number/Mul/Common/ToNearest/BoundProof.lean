...
The bound `5/(2^63 + 7)` is the tight supremum (see `operator_mul_rounding_bound_to_nearest_attained`). -/
theorem operator_mul_rounding_bound_to_nearest (x y : Number) (result : Number)
