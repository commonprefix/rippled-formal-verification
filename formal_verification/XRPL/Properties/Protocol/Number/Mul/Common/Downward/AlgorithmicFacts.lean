...
/-! ## Algorithmic decomposition for `.downward`-mode `operator_mul`. -/

theorem operator_mul_algorithmic_facts_downward (x y result : Number)
    (hx : x.isNormalized) (hy : y.isNormalized)
    (hx_mant_ne : x.mantissa_ ≠ 0) (hy_mant_ne : y.mantissa_ ≠ 0)
    (hok : Number.operator_mul x y .downward = .ok result)
    (hresult : result.mantissa_ ≠ 0) :
    ∃ (zm : UInt64) (ze' : Int) (f : ℚ) (g : Guard) (res_pos : RoundResult),
      MulFactsSpec x y result .downward zm ze' f g res_pos := by
  obtain ⟨zn, zm, ze', f, g, res, hzn, hzm_succ, hzm_le_maxRep, hf_nn, hf_lt, hfloor,
      habs, hf_rep, hsbit, h_rup, hok'⟩ :=
    operator_mul_decompose x y result .downward hx hy hx_mant_ne hy_mant_ne hok
  ...
  have h_inv := doRoundUp_output_invariants_downward_upTo_maxRepUp g zn zm ze' hzm_ge hzm_le_maxRep .overflow res h_rup hres_mant_ne
  ...
  have h_rup_pos : g.doRoundUp false zm ze' largeRange.min largeRange.max .downward .overflow = .ok res_pos :=
    doRoundUp_false_from_ok g zn zm ze' .downward .overflow res h_rup
