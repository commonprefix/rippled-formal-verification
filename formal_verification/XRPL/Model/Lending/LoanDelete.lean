...
def deleteActiveLoan (vault : Vault) (broker : LoanBroker) : Except Error (Vault × LoanBroker) :=
...
-- LoanDelete -> doApply
def Loan.delete (loan : Loan) (vault : Vault) (broker : LoanBroker)
    : Except Error (Vault × LoanBroker) :=
